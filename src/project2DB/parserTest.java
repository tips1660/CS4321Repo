...
public class parserTest {
	...
	public static void main(String[] args) throws Exception {
		...
		dbcat.setTableToIndex(tableToIndex);
		dbcat.setIndexToCluster(indexToCluster);
		...
		PhysicalPlanBuilder buildPhys = new PhysicalPlanBuilder(joinType, joinBuffer, sortType, sortBuffer, useIndexes);
		...
		if(evaluateQueries == 1){
			try {
				...
				while ((statement = parser.Statement()) != null) {
					...
					//    deleteRecursive(f1);                
					//}
					out = output + File.separator +"query" + counter;
					try{
		...
		{
			System.out.println("chose to not evaluate queries");
		}
	}
